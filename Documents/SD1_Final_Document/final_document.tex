...
\begin{figure}[H]
  ...
  \includegraphics[scale=.75]{graphics/ISAM_abstract_diagram.png}
...
data structures to offer high scalability and guaranteed high throughput. In order to accomplish this
...
dubbed libomdb. On the server since the client is using SQL in order to provide
...
\par\vspace{\baselineskip}
Boost.Variant focuses on type-safe storage and retrieval of a bounded set of types that composes a 
discriminated union. A variant data type exposes an interface independent of the current value's type. 
The user defines the set of types that are allowed in a variant and this can contain any number of 
disparate variant instructions. Two ways to retrieve a type from the value that is held in a variant is 
either the program must store this type or use boost visitor mechanism. This variant data type is stored 
as a stack instead of a heap to avoid expensive memory allocations.
\par\vspace{\baselineskip}

{\bfseries BoundedType} :
\par\vspace{\baselineskip}
Every type that is specified as a template argument to variant must satisfy the above requirements. 
In addition, certain features of variant are available if its bounded types meet certain extra 
requirements. The minimum requirements for a type to be accepted in a boost variant object are:
\begin{enumerate}
...
const U & get(const variant<T1, T2, ..., TN> &);
  ...
  \caption{Describes the type of data variant will accept\copyright Friedman, Maman 2002, reprinted with permission.\cite{boostvariant}} 
  \label{templateboostvar}
\end{figure}
...
the heterogeneous data variant at declaration. If the \\ \inlinecode{boost::variant} object
changes its data type then \inlinecode{boost::get<T>} will lead to logical errors that were not considered.
...
function. As shown in Figures~\ref{typesequence} and~\ref{typesequence2}, you can create a 
type sequence that is customizable.
\par\vspace{\baselineskip}
...
\begin{figure}[H]
  ...
  \end{lstlisting}
  \caption{Allows the program to create their own type sequence without using variant constructor\copyright Friedman, Maman 2002, reprinted with permission.\cite{boostvariant}} 
  \label{typesequence}
\end{figure}
...
extends this by allowing simultaneous visitation of content of two different variant objects. An example is 
shown in Figure~\ref{binvisitation}.
\par\vspace{\baselineskip}
...
public:
  ...
  \end{lstlisting}
  \caption{Compares two variant objects \copyright Friedman, Maman 2002, reprinted with permission.\cite{boostvariant}} 
  \label{binvisitation}
\end{figure}
...
multiple compares between variants objects with in tables. An example code that performs multiple 
visitation can be found in Figure~\ref{code:multi_visit}.
...
struct if_visitor :
  public boost::static_visitor<arithmetics_t>
{
  ...
  \end{lstlisting}
  \cite{boostvariant}
  \caption{Compares three or more variant objects \copyright Friedman, Maman 2002, reprinted with permission} 
  \label{code:multi_visit}
\end{figure}
...
\begin{enumerate}
  ...
  \item \inlinecode{createTable} takes in a char array tablename and a \inlinecode{TableSchema} object in order to add
  the string to the hashmap lookup table as well as the table schema. This is added as a pair
  into the lookup table and now the table is registered into the lookup table and memory is allocated
  for that table
  \item \inlinecode{checkSchema} takes in a char array tablename and checks whether or not if the table exists in the
  hashmap lookup table. If it does exist then it checks whether the TableSchema schema matches the
  schema from the lookup table. If it does match up then it returns true otherwise false.
  \item \inlinecode{getRow} takes in a char array tablename and an int rowID in order to find the table in the lookup table
  and finds the row based on the rowID index. It then returns the row pointer it found
  \item \inlinecode{getTable} takes in a char array tablename and finds the table from the lookup table. If it finds the table
